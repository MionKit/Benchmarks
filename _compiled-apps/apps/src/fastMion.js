...
exports.initFsHttp = __assignType((app, handlersDataFactory, routerOptions) => {
    const serverFactory = __assignType((handler, opts) => {
        const server = (0, http_2.createServer)(__assignType((req, res) => {
            req.method = "POST";
            handler(req, res);
        }, ['req', 'res', '', 'P"2!"2""/#']));
        return server;
    ...
    fastify.post("*", __assignType((fsRequest, fsResponse) => {
