...
/**
 * Generate a sample user payload for benchmarking (complex User model).
...
/**
 * Generate a simple user payload for benchmarking (SimpleUser model).
 * Uses template literal to avoid JSON.stringify overhead on each request.
 * Only the id is dynamic, rest is static string.
 *
 * @param {boolean} isMion - Whether to wrap in array for mion RPC-style API
 * @returns {string} JSON string of the simple user payload
 */
