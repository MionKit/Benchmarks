...
module.exports.chartScreenshot = async function (
  ...
  if (doBase64) {
    ...
    jsonPath = join(chartsDirectory, `${chartName}.json`);
    fs.writeFileSync(jsonPath, JSON.stringify(options));
    console.log(`chart saved to ${path}`);

    // save chart options to json file so can be used for websites
  }
...
module.exports.getMarkdownBarChart = async function (
  ...
  const columnsData = outputResults.map((result) => [
    `${result.name}`,
    result[metricName] > 1000
      ? Math.floor(result[metricName])
      : result[metricName],
  ]);
